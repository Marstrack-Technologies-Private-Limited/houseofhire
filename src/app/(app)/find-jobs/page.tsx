      ...
      try {
        ...
        const jobsWithCompany = jobsResponse.data.map((job: Job) => ({
          ...
          RECRUITERCOMPANYNAME:
            recruitersMap.get(job.RECRUITERID) || "Unknown Company",
        }));
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter((job) => {
      ...
      const countryData = Country.getAllCountries().find(
        (c) => c.name === job.COUNTRY
      );
      const countryMatch =
        countryFilter === "all" ||
        (countryData && countryData.isoCode === countryFilter);
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Browse through the available job openings.
          </p>
        </div>
          ...
          <CardHeader>
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription>
                  Use the filters below to find your perfect job.
                </CardDescription>
              </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 border rounded-lg">
            ...
            <p className="text-muted-foreground mt-2">
              No jobs match your search criteria. Please check back later.
            </p>
          </div>
        ) : viewMode === "card" ? (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredJobs.map((job, index) => {
              ...
              return (
                <Card
                  key={`${job.REQUESTNO}-${index}`}
                  className="flex flex-col"
                >
                  <CardHeader>
                    <CardTitle>{job.DESIGNATION}</CardTitle>
                    <CardDescription>
                      {job.RECRUITERCOMPANYNAME} - {job.CITY}, {job.COUNTRY}
                    </CardDescription>
                  </CardHeader>
                  ...
                  <CardFooter className="flex flex-col items-start gap-2">
                    <p className="text-xs text-muted-foreground">
                      ...
                      {format(new Date(job.DEADLINEDATE), "PPP")}
                    </p>
                    <div className="flex w-full gap-2 items-center">
                      ...
                      <Button
                        variant="secondary"
                        className="w-full"
                        onClick={() => setIsTracking(job.REQUESTNO)}
                      >
                        Track Application
                      </Button>
                    </div>
